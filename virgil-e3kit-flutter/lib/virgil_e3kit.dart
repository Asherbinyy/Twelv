...
class Ethree {
  ...
  static Future<Ethree?> init(String identity, GetJWTCallback tokenCallback) async {
    final String channelID = getRandString(32);
    bool result =
        await globalChannel.invokeMethod('init', {'channelID': channelID, 'identity': identity});
    if (!result) {
      return null;
    }

    final MethodChannel localChannel = MethodChannel(channelID);
    final Ethree ethree = Ethree(identity, tokenCallback, localChannel);
  ...
  Future<bool?> changePassword(String oldPassword, String newPassword) async {
    return _channel
        .invokeMethod("changePassword", {"oldPassword": oldPassword, "newPassword": newPassword});
  }
  ...
  Future<String> findUser(String identity) async {
    final String card = await _channel.invokeMethod('findUser', {'identity': identity});

    return card;
  }

  Future<Map<dynamic, dynamic>> findUsers(List<String> identities, bool forceReload) async {
    Map<dynamic, dynamic> users = await _channel
        .invokeMethod("findUsers", {"identities": identities, "forceReload": forceReload});

    return users;
  }

  Future<String> findCachedUser(String identity) async {
    final String card = await _channel.invokeMethod("findCachedUser", {'identity': identity});

    return card;
  }

  Future<Map<dynamic, dynamic>> findCachedUsers(List<String> identities) async {
    final Map<dynamic, dynamic> users =
        await _channel.invokeMethod("findCachedUsers", {"identities": identities});
  ...
  Future<dynamic> handleMethodCall(MethodCall call) {
    ...
    throw "Method ${call.method} not supported";
  }
}
