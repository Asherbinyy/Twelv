...
class EThreeWrapper(private val identity: String, private val channelID: String, private val messenger: BinaryMessenger, private val context: Context, private val activity: Activity) : MethodCallHandler, Result {
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    ...
    try {
      when (call.method) {
        ...
        // Add other method cases here
        else -> currentResult.error("0", "Method is not implemented", call.method)
      }
    } catch (e: Exception) {
      currentResult.error("-1", e.message, call.method)
    }
  }

  private fun getIdentity() {
    currentResult.success(this.ethree.identity)
  }

  private fun register() {
    this.ethree.register().addCallback(completeCallback())
  }

  private fun unregister() {
    this.ethree.unregister().addCallback(completeCallback())
  }

  // Add other function definitions here

  private fun completeCallback(): OnCompleteListener {
    return object : OnCompleteListener {
      ...
      override fun onError(throwable: Throwable) {
        activity.runOnUiThread {
          currentResult.error("2007", throwable.message, null)
        }
      }
    }
  }

  // Implement the Result interface methods
  override fun success(result: Any?) {
    currentResult.success(result)
  }

  override fun error(errorCode: String?, errorMessage: String?, errorDetails: Any?) {
    currentResult.error(errorCode, errorMessage, errorDetails)
  }
